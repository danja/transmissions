...
describe('Test Application Runner', function () {
    ...
    commands.forEach((test, index) => {
        const { command, description, requiredMatchCount } = test;

        it(`should run the app, one pass report in results for command ${index + 1}`, (done) => {
            console.log(`Running test: ${description}`); // Print description to console
